...
* **rotate(img, angle)** -> Rotates `img` around its center by `angle` degrees (current implementation supports multiples
  of 90; positive is counter-clockwise, negative is clockwise). For 90/270 rotations, output dimensions are swapped so
  the full image is preserved (no clipping)
